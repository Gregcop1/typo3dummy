...
class tx_irfaq_pi1 extends tslib_pibase {
				...
				foreach ($rows as $row) {
					...
					if (($row = $this->getLanguageOverlay('tx_irfaq_q', $row))) {
						$markers = array(
							...
							'###RELATED_FAQ_ENTRY_HREF###' => $this->pi_list_linkSingle('', $row['uid'], true, array(), true),
						);
	...
	function fillMarkerArrayForRow(&$row, $i) {
		...
		$markerArray['###SINGLEVIEW_LINK###'] = $this->pi_list_linkSingle('', $row['uid'], true, array(), true);
	...
	function singleView() {
		...
		if (count($rows) == 0) {
			...
			$markers['###BACK_HREF###'] = $this->cObj->typoLink_URL(array('parameter'=>$GLOBALS['TSFE']->id));
